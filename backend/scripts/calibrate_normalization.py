...
This development tool calibrates base-time normalization only. Congestion delay
is handled separately by the runtime cost function. It never updates production
...
def main() -> None:
    ...
    base_times = [edge.base_time_min for edge in edges]

    distance_statistics = calculate_statistics(distances)
    time_statistics = calculate_statistics(base_times)
    ...
    print()
    print_statistics("Base time (min)", time_statistics)
    print()
    print(f"Recommended MAX_TIME_MIN: {recommended_time:.1f}")
    print(
        "Edges above recommended time limit: "
        f"{sum(value > recommended_time for value in base_times)}/{len(edges)}"
    )
    print()
